  ...
  return (
    ...
    <h6>Customer Reviews</h6>
      {customerReview?.map((rating, index) => (
        <div key={index}>
          <Link className='text-decoration-none text-dark'>{rating.clear}</Link>
      ...
      <div className='ps-3'>
        <h6>Price</h6>
        {
          priceSortList?.map((price)=>(
            ...
            <Link className='text-decoration-none text-dark'>{price?.clear}</Link>
            <Link className='text-decoration-none text-dark fw-bold'>{price?.below}</Link>
            {price?.price?.map((pri)=><Link className='text-decoration-none text-dark'>{pri}</Link>)}
            </div>
          ))
        }
        ...
        <ButtonTag label={'Go'} variant={'outline-info'} className={'text-dark'}/>
        </div>
      </div>
      <div className='ps-3'>
        <h6>Discount
        </h6>
        {discountOffer?.map((off)=><Link className='text-decoration-none text-dark d-flex flex-column'>{off} Off or more</Link>)}
      </div>
    </>
  )
}

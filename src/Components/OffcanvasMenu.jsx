  ...
  return (
    <>
     <Offcanvas show={show} onHide={handleClose}>
        <Offcanvas.Header style={{backgroundColor:'#232f3e'}} className='text-white'>
        ...
        <Offcanvas.Body className='p-0'>
          {menuList?.map((option,index)=>(
            <ul key={index} className='footer-list p-0'>
              ...
              <li className='d-flex flex-column gap-4 ps-4 '>{option?.menu?.map((li,i)=>(<li key={i} className='canvas-hover'>{li}</li>))}</li>
              <hr className='w-100'/>
            </ul>
          ))}
        </Offcanvas.Body>

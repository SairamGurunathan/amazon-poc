    ...
    <><div>
      ...
      <div className="text-white footer-bottom">
        ...
        {AdditionalLink?.map((add, i) => (
          ...
          <ul key={i} className="footer-list">
              <li>
                ...
                <small className="text-wrap text-mute fs-12 w-50">{add?.discription}</small>
                </div>
                </Link>
              </li>
          </ul>
        ...
        <div className="d-flex flex-row align-items-center justify-content-center gap-2">
          {policy?.map((term,i)=>(<Link className="text-decoration-none text-white link-hover"><small className="fs-12">{term}</small></Link>))}
          </div>
        <small className="d-flex align-items-center justify-content-center pb-4 fs-12">© 1996-2024, Amazon.com, Inc. or its affiliates</small>
        </Container>
      </div>
      </div>
    </>
